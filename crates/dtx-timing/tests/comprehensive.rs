...
fn bpm_change_struct_basic() {
    let c = BpmChange { measure: 2, bpm: 180.0 };
...
fn bpm_change_struct_zero_measure() {
    let c = BpmChange { measure: 0, bpm: 120.0 };
...
fn bpm_change_struct_equality() {
    let a = BpmChange { measure: 1, bpm: 120.0 };
    let b = BpmChange { measure: 1, bpm: 120.0 };
    let c = BpmChange { measure: 2, bpm: 120.0 };
    assert_eq!(a, b);
    assert_ne!(a, c);
}
...
fn bpm_change_high_bpm() {
    let c = BpmChange { measure: 1, bpm: 999.0 };
    assert_eq!(c.bpm, 999.0);
}

#[test]
fn bpm_change_low_bpm() {
    let c = BpmChange { measure: 1, bpm: 30.0 };
    assert_eq!(c.bpm, 30.0);
}

#[test]
fn bpm_change_very_high() {
    let c = BpmChange { measure: 5, bpm: 1000.0 };
    assert_eq!(c.bpm, 1000.0);
}

#[test]
fn bpm_change_very_low() {
    let c = BpmChange { measure: 5, bpm: 1.0 };
    assert_eq!(c.bpm, 1.0);
}

#[test]
fn bpm_change_clone() {
    let c = BpmChange { measure: 1, bpm: 120.0 };
    let c2 = c;
    assert_eq!(c, c2);
}
