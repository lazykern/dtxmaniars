...
use crate::{lane_owner, BindSource, InputBindings, SystemVerb, BINDABLE_CHANNELS, SYSTEM_VERBS};
use bevy::prelude::*;
...
pub struct ActiveInputProfiles {
    /// The active keyboard profile.
    pub keyboard: KeyboardProfile,
    /// The active MIDI profile.
...
#[derive(Resource, Debug, Clone, Default)]
pub struct LiveBindings(pub InputBindings);
...
/// Path to the on-disk keyboard profile registry.
...
/// Path to the on-disk MIDI profile registry.
...
///
/// Called from gameplay-drums's `editor::profile_bar_ui` via the
/// `crate::bindings::` shim.
pub fn active_keyboard_profile(registry: &ProfileRegistry<KeyboardProfile>) -> KeyboardProfile {
    registry
        .profiles
        .get(&registry.active)
        .cloned()
...
/// Look up a registry's active MIDI profile value, falling back to
/// built-ins then the code default.
///
/// Called from gameplay-drums's `editor::profile_bar_ui` via the
/// `crate::bindings::` shim.
pub fn active_midi_profile(registry: &ProfileRegistry<MidiProfile>) -> MidiProfile {
    registry
        .profiles
        .get(&registry.active)
        .cloned()
...
/// Called from gameplay-drums's `editor::profile_bar_ui` via the
/// `crate::bindings::` shim.
pub fn compose_bindings(keyboard: &KeyboardProfile, midi: &MidiProfile) -> InputBindings {
    let mut bindings = InputBindings {
        midi: crate::MidiDeviceConfig {
...
///
/// Scheduled by gameplay-drums's `bindings::plugin` (dtx-input cannot
/// reference `game_shell::AppState`).
pub fn reload_profiles(
    ...
    let legacy = crate::default_bindings_path();
...
/// Scheduled by gameplay-drums's `bindings::plugin` (dtx-input cannot
/// reference `game_shell::AppState`).
pub fn apply_live_bindings(live: Res<LiveBindings>, mut resolver: ResMut<BindResolver>) {
...
mod tests {
    ...
    fn custom_binding_reroutes_lane() {
        ...
        b.bind(EChannel::Snare, crate::BindSource::Key(KeyCode::KeyX));
    ...
    fn shared_key_binding_maps_to_multiple_lanes() {
        let mut b = InputBindings::default();
        b.bind_shared(EChannel::Snare, crate::BindSource::Key(KeyCode::KeyX));
        let r = BindResolver::from_bindings(&b);
        assert_eq!(
    ...
    fn resolver_tracks_live_binding_edit() {
        let mut ib = crate::InputBindings::default();
        let sd = dtx_core::EChannel::Snare;
        ib.bind(sd, crate::BindSource::Key(KeyCode::KeyP));
    ...
    fn system_binds_survive_split_and_compose() {
        use crate::{profiles::split_bindings, SystemVerb};
    ...
    fn failed_registry_selection_keeps_active_resolver() {
        use crate::profiles::{reduce_registry, RegistryAction};
        let registry = crate::profiles::keyboard_registry();
