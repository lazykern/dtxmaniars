//! Note spawning + scrolling.
//!
//! Two systems:
//! - `spawn_notes_system`: reads `ActiveChart`, spawns a `Note` entity for
//!   each chip whose `target_ms` is within `[now, now + lookahead]`.
//! - `scroll_notes_system`: moves existing note entities downward over time.
//! - `despawn_missed_notes_system`: emits `NoteMissed` and despawns.
//!
...
fn y_for_target(target_ms: i64, now_ms: i64) -> f32 {
    // Notes above hit line spawn with positive offset; notes below = past target.
...
fn spawn_notes_system(
    ...
    let Some(now) = clock.current_ms else { return };
...
fn despawn_missed_notes_system(
    ...
    let Some(now) = clock.current_ms else { return };
    let mut hit_lanes: Vec<bool> = vec![false; 9];
