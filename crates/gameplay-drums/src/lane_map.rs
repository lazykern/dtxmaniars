//! Lane order + channel↔lane helpers.
//!
//! Keybinds no longer live here — input sources are resolved from
//! `dtx-config::bindings` (`InputBindings`) via `crate::bindings::BindResolver`.
//! This module owns only the fixed BocuD lane order and channel↔lane mapping,
//! used by judge/autoplay/hit_sound/drum_groups/hud.
//!
